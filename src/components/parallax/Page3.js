...
function Page3() {
  return (
    <div className="h-screen w-full relative flex items-center overflow-hidden">
      ...
      <div
        className="h-screen w-full absolute bottom-0"
        style={{
      ...
      <Image
        className="absolute bottom-0  object-top  z-10 w-[60%] max-h-[80%] "
        style={{ objectFit: "cover", minHeight: "70%", objectPosition: "top" }}
        src={fort}
      />
      <Image
        className="absolute top-[20%] left-[13%] w-[240px] z-0"
        src={moon}
      />
      <Image className="absolute top-32 right-2 scale-50 z-0" src={cloudBig} />
      <Image
        className="absolute top-24 right-40 scale-50 z-0"
        src={cloudSmall}
      />
      <p
        className="absolute top-[25%] right-60 text-transparent bg-clip-text font-sankaran text-4xl md:text-7xl "
        style={{
          background:
            "linear-gradient(180deg, #071627 15.73%, #67B3D6 157.53%)",
        }}
      >
        Events
      </p>
